$(document).ready(function () {
  ...
  function searchItems(query) {
    ...
    if (query === '') {
      loadItems('rest/lost-items', 'Lost');
      loadItems('rest/found-items', 'Found');
      return;
    }

    RestClient.get(`rest/lost-items/name/${query}`, function (items) {
      if (Array.isArray(items) && items.length > 0) {
        items.forEach(item => {
    ...
    RestClient.get(`rest/found-items/name/${query}`, function (items) {
      if (Array.isArray(items) && items.length > 0) {
        items.forEach(item => {
